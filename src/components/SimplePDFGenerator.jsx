...
const PrintableItinerary = React.forwardRef(({ formData }, ref) => {
  ...
  return (
    <div ref={ref} className="max-w-4xl mx-auto p-8 bg-white print:p-4">
      {/* Print styles */}
      <style jsx>{`
        @media print {
          @page {
...
const SimplePDFGenerator = ({ formData }) => {
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      ...
      <div style={{ display: 'none' }}>
        <PrintableItinerary ref={componentRef} formData={formData} />
      </div>
      
      {/* Print button */}
      <button
        onClick={handlePrint}
        className="px-6 py-3 bg-purple-800 text-white rounded-lg hover:bg-purple-900 transition font-medium"
      >
        Generate PDF
      </button>
      
      <p className="text-sm text-gray-600 text-center">
        Click the button above to generate and download your itinerary as PDF
      </p>
    </div>
  );
};
